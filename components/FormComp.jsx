...
const FormComp = ({ dept1, dept2, isLoading, setIsLoading }) => {
  ...
  const handleSubmit = async (values) => {
    ...
    const submitDepartment = async (department) => {
      ...
      // BUG FIX: `Pref` ("1st"/"2nd" preference) is displayed as its own
      // column in the admin table and CSV export (components/DataTable.jsx,
      // constants/index.js CSV_Header, lib/modals/form.modal.ts) but was
      // never actually populated by this submission flow, so it was always
      // blank for every applicant. It's derived here from the department's
      // position in `departmentNames`, which reflects the order the
      // applicant chose departments in (see the join-page fix above).
      const preference = departmentNames.indexOf(department) === 0 ? "1st" : "2nd";
